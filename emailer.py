...
def email_sender(requirements, report):
    ...
    msg = MIMEMultipart()
    msg["Subject"] = "Report: {}, {}".format(report.replace("_", " "), d)
    msg["From"] = username
    ...
    part.add_header(
        "Content-Disposition", "attachment; filename={}_{}.csv".format(report, d)
    )
...
def date_validator(requirements):
    ...
    if (
        requirements["frequency"] == "dow"
        and dow_actual in requirements["day_validator"]
    ):
        should_run = True
    elif (
        requirements["frequency"] == "dom"
        and dom_actual in requirements["day_validator"]
    ):
        should_run = True
    elif (
        requirements["frequency"] == "qrt"
        and dom_actual in requirements["day_validator"]
        and qrt_actual in [1, 4, 7, 10]
    ...
    for script in scripts:
        ...
        if script in schedule_manager:
            if date_validator(schedule_manager[script]):
                ...
                csv_writer(
                    headers=colnames, data=response, report=script.replace(".sql", "")
                )
                email_sender(
                    schedule_manager[script], report=script.replace(".sql", "")
                )
            else:
